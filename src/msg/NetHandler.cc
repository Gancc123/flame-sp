...
int NetHandler::create_socket(int domain, bool reuse){
    ...
    if(sfd < 0){
        r = errno;
        ML(fct, error, "create socket failed. {}", cpp_strerror(r));
        return -r;
    }
    int on = 1;
    if (::setsockopt(sfd, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on)) == -1) {
        r = errno;
        ML(fct, error, "setsockopt SO_REUSEADDR failed: {}", cpp_strerror(r));
        close(sfd);
        return -r;
    }
    return sfd;
...
int NetHandler::set_nonblock(int fd){
    ...
    * interrupted by a signal. */
    if ((flags = fcntl(fd, F_GETFL)) < 0 ) {
        r = errno;
        ML(fct, error, "fcntl(F_GETFL) failed: {}", cpp_strerror(r));
        return -r;
    }
    if (fcntl(fd, F_SETFL, flags | O_NONBLOCK) < 0) {
        r = errno;
        ML(fct, error, "fcntl(F_SETFL,O_NONBLOCK) failed: {}", cpp_strerror(r));
        return -r;
    }
    return 0;
}
...
void NetHandler::set_close_on_exec(int fd){
    ...
    if (flags < 0) {
        int r = errno;
        ML(fct, error, "fcntl(F_GETFD) failed: {}", cpp_strerror(r));
        return;
    }
    if (fcntl(fd, F_SETFD, flags | FD_CLOEXEC)) {
        int r = errno;
        ML(fct, error, "fcntl(F_SETFD) failed: {}", cpp_strerror(r));
    }
}
...
int NetHandler::set_socket_options(int fd, bool nodelay, int size){
    ...
    if (nodelay) {
        ...
        if (r < 0) {
            r = errno;
            ML(fct, error, "couldn't set TCP_NODELAY: {}", cpp_strerror(r));
        }
    }
    if (size) {
        ...
        if (r < 0)  {
            r = errno;
            ML(fct, error, "couldn't set SO_RCVBUF to {}:{}", 
                                                    size , cpp_strerror(r));
        }
    }
...
int NetHandler::accept(int sfd, NodeAddr &caddr){
    ...
    if(cfd < 0){
        r = errno;
        //ML(fct, error, "accept failed: {}", cpp_strerror(r));
        return -r;
    }
...
int NetHandler::get_sockname(int fd, NodeAddr &addr){
    ...
    if(r < 0){
        r = errno;
        ML(fct, error, "getsockname failed: {}", cpp_strerror(r));
        return -r;
    }
