...
bool FlameContext::init_config(const std::string& path) {
    ...
    if (config == nullptr) {
        log()->lerror("FlameContext", "read config file (%s) faild.", path.c_str());
        return false;
    }
    set_config(config);
    return true;
...
bool FlameContext::init_log(const std::string& dir, const std::string& level, const std::string& prefix) {
    ...
    if (dir.empty()) {
        if (config()->has_key(CFG_LOG_DIR)) {
            ...
            log()->lerror("FlameContext", "missing config item '%s'", CFG_LOG_DIR);
            return false;
        }
    } else {
        log_dir = dir;
    ...
    if (!log()->set_level_with_name(level)) {
        log()->lerror("FlameContext", "set log level faild");
        return false;
    }
    
    if (!log()->reopen(log_dir, prefix)) {
        log()->lerror("FlameContext", "reopen logger to file faild");
        return false;
    }
    return true;
}
