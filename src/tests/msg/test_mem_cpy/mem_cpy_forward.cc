...
#define CFG_PATH "flame_forward.cfg"
...
int main(int argc, char *argv[]){
    ...
    if(!fct->init_log("", msg::str2upper(std::string(options.get("log_level"))), 
                fmt::format("forward{}",  std::string(options.get("index"))))){
    ...
    cycles_t start , end;
    cfg.tposted[0] = get_cycles();
    start = cfg.tposted[0];
    msger->send_req_msg(0);
    end = get_cycles();
    msg::print_time(start, end, "send req msg: {}us");
