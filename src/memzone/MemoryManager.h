#ifndef FLAME_MEMORY_RDMA_MEM_POOL_H
#define FLAME_MEMORY_RDMA_MEM_POOL_H
...
class MemoryManager{
public:
    MemoryManager(FlameContext *c, flame::msg::ib::ProtectionDomain *p, MemoryConfig *_cfg);
    ~MemoryManager();
    ...
    flame::msg::ib::ProtectionDomain *get_pd() const{
        return pd;
    }
    ...
    bool can_alloc(unsigned nbufs){
        ...
        if(mem_cfg->rdma_buffer_num <= 0){
            return true;
        } 
        if(mem_cfg->rdma_buffer_num < (n_bufs_allocated + nbufs)){
            fct->log()->lerror("rdma buffer number reach limit: {}, need{}, allocated:{}", 
                                mem_cfg->rdma_buffer_num, nbufs, n_bufs_allocated);
            //ML(mct, error, 
...
private:
    ...
    static uint64_t calcu_init_space(MemoryConfig *mem_cfg);
